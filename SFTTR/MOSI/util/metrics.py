...
def get_all_pairs_indices(labels, ref_labels=None):
    ...
    #按位异或，得到matches的反向
    diffs = matches ^ 1
    #如果ref_labels和labels是同一个对象，那么将matches的对角线元素设置为0。
    if ref_labels is labels:
        matches.fill_diagonal_(0)
    #找出matches和diffs中为True的元素的索引，分别作为正样本对和负样本对的索引。
    a1_idx, p_idx = torch.where(matches)
    a2_idx, n_idx = torch.where(diffs)
    return a1_idx, p_idx, a2_idx, n_idx


#根据输入的索引元组生成锚点-正样本和锚点-负样本的索引
#用于训练对比学习模型
def convert_to_pairs(indices_tuple, labels):
    ...
    if indices_tuple is None:
        ...
        a, p, n = indices_tuple
        #对于每个正样本，我们使用相同的锚点来生成负样本。
        return a, p, a, n

##计算对比学习的损失，通过调整样本对的权重和距离来实现
