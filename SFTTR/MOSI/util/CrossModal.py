...
class CrossEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.position_embeddings = nn.Embedding(config.MOSI.downStream.max_position_embeddings, config.MOSI.downStream.hidden_size)
        self.token_type_embeddings = nn.Embedding(config.MOSI.downStream.type_vocab_size, config.MOSI.downStream.hidden_size)

        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = LayerNorm(config.MOSI.downStream.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.MOSI.downStream.hidden_dropout_prob)
...
class CrossEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(config.MOSI.downStream.num_hidden_layers)])
...
class CrossOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.MOSI.downStream.intermediate_size, config.MOSI.downStream.hidden_size)
...
class CrossSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if config.MOSI.downStream.hidden_size % config.MOSI.downStream.num_attention_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (config.MOSI.downStream.hidden_size, config.MOSI.downStream.num_attention_heads))

        self.num_attention_heads = config.MOSI.downStream.num_attention_heads  #8 or 12
        self.attention_head_size = int(config.MOSI.downStream.hidden_size / config.MOSI.downStream.num_attention_heads) #768/8 96
        self.all_head_size = self.num_attention_heads * self.attention_head_size   # 8*96=768
        #定义三个线性变换，分别用于生成查询、键和值
        self.query = nn.Linear(config.MOSI.downStream.hidden_size, self.all_head_size) #torch.Size([768, 768])
        self.key = nn.Linear(config.MOSI.downStream.hidden_size, self.all_head_size) #torch.Size([768, 768])
        self.value = nn.Linear(config.MOSI.downStream.hidden_size, self.all_head_size) #torch.Size([768, 768])
        #用于在训练过程中随机丢弃一部分神经元，以防止过拟合。
        self.dropout = nn.Dropout(config.MOSI.downStream.attention_probs_dropout_prob) #0.1 or 0.3
...
class CrossSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.MOSI.downStream.hidden_size, config.MOSI.downStream.hidden_size)
        #层归一化
        self.LayerNorm = LayerNorm(config.MOSI.downStream.hidden_size, eps=1e-12)
...
class CrossIntermediate(nn.Module):
    def __init__(self, config):
        ...
        #全连接层，输入输出为256
        self.dense = nn.Linear(config.MOSI.downStream.hidden_size, config.MOSI.downStream.intermediate_size)
        ...
        self.intermediate_act_fn = ACT2FN[config.MOSI.downStream.hidden_act] \
            if isinstance(config.MOSI.downStream.hidden_act, str) else config.MOSI.downStream.hidden_act
