...
class TextEncoder(nn.Module):
    def __init__(self, name=None, fea_size=None, proj_fea_dim=None, drop_out=None, with_projector=True,
        ...
        if config.USEROBERTA:
            ...
            self.extractor = BertModel.from_pretrained("hfl/chinese-roberta-wwm-ext")
        else:
            self.tokenizer = BertTokenizer.from_pretrained('/data/tm/ConFEDE-Graph_Main/bert-base-chinese')
            self.extractor = BertModel.from_pretrained('/data/tm/ConFEDE-Graph_Main/bert-base-chinese')
...
class TextPretrain(nn.Module):
    ...
    def save_model(self, name):
        # save all modules
        encoder_path = self.config.SIMS.path.encoder_path + name + '_text_encoder.ckpt'
        decoder_path = self.config.SIMS.path.encoder_path + name + '_text_decoder.ckpt'
