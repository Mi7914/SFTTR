...
class TVA_fusion(nn.Module):
    def __init__(self, name=None, encoder_fea_dim=None, drop_out=None, config=default_config):
        ...
        self.T_simi_proj = common_feature_extractor(encoder_fea_dim, int(encoder_fea_dim / 2))
        self.V_simi_proj = common_feature_extractor(encoder_fea_dim, int(encoder_fea_dim / 2))
        self.A_simi_proj = common_feature_extractor(encoder_fea_dim, int(encoder_fea_dim / 2))

        self.T_dissimi_proj = private_feature_extractor(encoder_fea_dim, int(encoder_fea_dim / 2))
        self.V_dissimi_proj = private_feature_extractor(encoder_fea_dim, int(encoder_fea_dim / 2))
        self.A_dissimi_proj = private_feature_extractor(encoder_fea_dim, int(encoder_fea_dim / 2))
        ...
        self.TVA_decoder = BaseClassifier(input_size=int(encoder_fea_dim/2),
    ...
    def forward(self, sample1, sample2, return_loss=True, return_emb=False, device=None, return_all_fea=False):
        ...
        x_sds = x1_sds
        #全部标签
        label_sds = torch.cat((label_T1, label_V1, label_A1,label1,label1,label1), dim=0)
        x2 = None
        # x = x1_all
        #融合后的特征
        x = cross_output_1.mean(dim=1)
        label_all = label1 # 多模态的标签
