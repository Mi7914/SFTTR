...
def update_matrix(data, model, config=default_config):
    with torch.no_grad():
        model.eval()
        train_data = SIMSDataloader('train', batch_size=32, use_similarity=False, simi_return_mono=False, shuffle=False,
                                    use_sampler=False)
...
def TVA_train_fusion(name, load_model=None, check=None, model_type='all', load_pretrain=True,
    ...
    if check is None:
        ...
        check = check.copy()
    log_path = config.LOGPATH + "SIMS_TVA_experiment." + datetime.datetime.now().strftime(
        '%Y-%m-%d-%H%M') + '.log'
    metrics = Metrics()
    ...
    lr = config.SIMS.downStream.TVAExp_fusion.lr
    total_epoch = config.SIMS.downStream.TVAExp_fusion.epoch
    decay = config.SIMS.downStream.TVAExp_fusion.decay
    num_warm_up = config.SIMS.downStream.TVAExp_fusion.num_warm_up
    finetune_epoch = config.SIMS.downStream.TVAExp_fusion.finetune_epoch

    train_data = SIMSDataloader('train', batch_size=batch_size, use_similarity=True, simi_return_mono=False,
                                use_sampler=False)
    ...
    for epoch in range(1, total_epoch + 1):
        ...
        log = 'TVA_%s_ValidAcc\n\tEpoch:%d\n\tacc_2:%s\n\tF1_score:%s\n\tacc_3"%s\n\t' \
              'acc_5:%s\n\tMAE:%s\n\tCorr:%s\n\tLoss:%s\n' \
              '------------------------------------------' % (model_type,
                                                              epoch, result['Mult_acc_2'], result['F1_score'],
...
def eval(model, metrics=None, eval_data=None, device=None, config=default_config):
    with torch.no_grad():
        ...
        if eval_data is None: eval_data = SIMSDataloader('test', shuffle=False, num_workers=0,
            ...
            eval_data = SIMSDataloader(eval_data, shuffle=False, num_workers=0,
                                       batch_size=config.SIMS.downStream.TVAExp_fusion.batch_size)
        ...
        for index, sample in enumerate(bar):
            ...
            truth.append(label)
            loss += _loss.item() * config.SIMS.downStream.TVAExp_fusion.batch_size
...
def TVA_test_fusion(name, check_list=None, model_type='all', config=default_config):
    ...
    seed = config.seed
    log_path = config.LOGPATH + "SIMS_TVA_experiment_Test." + datetime.datetime.now().strftime(
