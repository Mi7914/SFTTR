...
class TextEncoder(nn.Module):
    def __init__(self, name=None, fea_size=None, proj_fea_dim=None, drop_out=None, with_projector=True,
        ...
        self.name = name
        if fea_size is None:
            fea_size = config.MOSEI.downStream.text_fea_dim
        if proj_fea_dim is None:
            proj_fea_dim = config.MOSEI.downStream.proj_fea_dim
        if drop_out is None:
...
class TextPretrain(nn.Module):
    ...
    def save_model(self, name):
        # save all modules
        encoder_path = self.config.MOSEI.path.encoder_path + name + '_text_encoder.ckpt'
        decoder_path = self.config.MOSEI.path.encoder_path + name + '_text_decoder.ckpt'
